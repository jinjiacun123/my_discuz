...
function profile_show($fieldid, $space=array(), $getalone = false) {
	...
	if($fieldid=='gender') {
		...
		$job_time_year = $space['birthyear'].'-'.$space['birthmonth'].'-'.$space['birthday'];
        $return_job_time_year = diffDate($job_time_year);

		return $return_job_time_year;
		///return $return;
